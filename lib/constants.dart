import 'package:flutter/material.dart';

// list of colors that we use in our app
const kDarkBlueColor = Color(0xFF035AA6);
const kTextColor = Color(0xFF000839);
const kTextLightColor = Color(0xFF747474);
const kBlueColor = Color(0xFF40BAD5);
const kBackgroundColor = Color(0xFFe5e5e5);
const kSecondaryColor= Color(0xFFea5455);
const kPrimaryColor = Color(0xFF2d4059);
const kTextColorDark = Color(0xFF241424);
const kYellowColor = Color(0xFFFFA41B);
const kMahgony = Color(0xFF6D597A);
//const kPrimaryColor = Color(0xFF355070);
//const kSecondaryColor = Color(0xFFEAAC8B);
const kSecondaryVibrant = Color(0xFFB56576);
const kPrimaryVibrant = Color(0xFFE56B6F);
const kAppBarColor = Color(0xFF232f3e);
const kPurpleBackgroundColor = Color(0xFFe0d9fb);




const kDefaultPadding = 20.0;

// our default Shadow
const kDefaultShadow = BoxShadow(
  offset: Offset(0, 15),
  blurRadius: 27,
  color: Colors.black12, // Black color with 12% opacity
);
// My Text Styles
const kHeadingextStyle = TextStyle(
  fontSize: 28,
  color: kTextColor,
  fontWeight: FontWeight.bold,
);
const kSubheadingextStyle = TextStyle(
  fontSize: 24,
  color: Color(0xFF61688B),
  height: 2,
);

const kTitleTextStyle = TextStyle(
  fontSize: 20,
  color: kTextColor,
  fontWeight: FontWeight.bold,
);

const kSubtitleTextStyle = TextStyle(
  fontSize: 18,
  color: kTextColor,
  // fontWeight: FontWeight.bold,
);
const kHeadingextStyleBlue = TextStyle(
  fontSize: 28,
  color: kPrimaryColor,
  fontWeight: FontWeight.bold,
);

const kTitleTextStyleBlue = TextStyle(
  fontSize: 20,
  color: kPrimaryColor,
  fontWeight: FontWeight.bold,
);


const kAnimationDuration = Duration(milliseconds: 200);


